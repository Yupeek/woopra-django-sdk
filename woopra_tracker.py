...
except ImportError:  # Python 2 retro-compatibility
...
class WoopraTracker:
    ...
    def __init__(self, request):
        ...
        self.current_config["cookie_value"] = self.request.COOKIES.get(
            self.current_config["cookie_name"],
            self.random_cookie(),
        )
    ...
    def woopra_http_request(self, is_tracking, event=None):
        ...
        Parameters:
            isTracking - boolean :
            is this request supposed to track an event or just identify the user?
            event (optional) - dict :
            only matters if isTracking == True. The event to pass. Default is None.
        Result:
            None
        """
        ...
        if not is_tracking:
            url = '/track/identify/?{0}&&ce_app={1}'.format(
                urlencode(get_params),
                WoopraTracker.SDK_ID,
            )
        else:
            if event[0] is None:
        ...
        try:
            protocol = 'https' if self.current_config['ssl_tracking'] else 'http'
    ...
    def js_code(self):
        """
        This method returns the woopra code string to be passed to the Template,
        and outputed in its header.
        Parameters:
            None
        Result:
        ...
        txt_file = open(path + '/woopra.txt', 'r')
        code = txt_file.readline()
        txt_file.close()
        ...
        if self.has_pushed:
            code += "      woopra.push();"
        code += "   </script><!-- Woopra code ends here -->"
        return code
    ...
    def identify(self, user):
        ...
        Example:
            identify({'email': 'johndoe@mybusiness.com',
                     'name': 'John Doe', 'company': 'My Business'})
        """
        self.user = user
    ...
    def track(self, event_name=None, event_data=None, back_end_tracking=False):
        ...
        Parameters:
            event_name (optional) - str :
                The name of the event. If none is specified, will track pageview
        ...
        Examples:
            ...
            woopra.track("play", {'artist' : 'Dave Brubeck',
                         'song' : 'Take Five', 'genre' : 'Jazz'})
            # Track a custom event through the back-end:
            woopra.track('signup', {'company': 'My Business',
                         'username': 'johndoe', 'plan': 'Gold'}, True)
        """
    ...
    def push(self, back_end_tracking=False):
        """
        Pushes the indentification information on the user to Woopra
        in case no tracking event occurs.
        Parameter:
            back_end_tracking (optional) - boolean :
            Should the information be pushed through the back-end? Defaults to False.
        Result:
            None
        """
        if not self.user_up_to_date:
    ...
    def set_woopra_cookie(self, response):
        ...
        response.set_cookie(
            self.current_config["cookie_name"],
            self.current_config["cookie_value"],
    ...
    def actualize_cookie(self):
        """
        If the cookie name changes, call this method to see
        if a cookie matches the new name, and update cookie_value.
        Parameter:
            None
        Result:
            None
